...
class SubprocessInterpreter(BaseInterpreter):
    ...
    async def arun(
        ...
        try:
            ...
            code_name = f'Code Generated: {temp_file_path.name}'
            if(int(os.environ.get('START_CHAINLIT', 0))):
                import chainlit as cl
                elements = [
                    cl.Text(name=code_name, content=code, display="side", language=code_type,),
                    cl.File(name=temp_file_path.name, path=str(temp_file_path), display="inline"),
                ]
                await cl.Message(
                    content=code_name,
                    elements=elements,
