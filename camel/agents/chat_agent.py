...
class ChatAgent(BaseAgent):
    ...
    async def _aexecute_tool(
        ...
        tool = self._internal_tools[func_name]
        import asyncio
        logger.info(f"⚒️ Agent {self.role_name} executing tool {func_name} with arguments: {args}")
        try:
            if(int(os.environ.get('START_CHAINLIT', 0))):
                import chainlit as cl
                async with cl.Step(name=func_name, type="tool", default_open=True, show_input=False) as step:
                    step.input = json.dumps(args, ensure_ascii=False, indent=4)
                    ...
                    if(isinstance(result, str)):
                        if(len(result) >= 3000):
                            truncted_result = result[:3000] + '...'
                            step.output = truncted_result
                        else:
                            from toolkits import yield_word
                            async for text in yield_word(result, speed=1e-3):
